...
## 已完成 (Done)
- [026b - 工作台改版](tasks/026b-workbench-rework.md) — 通过（feature/026b-workbench-rework，**未 commit/push**，Codex 第2轮没 token 由 Claude 接手完成+验证）。工作台改 **2 格宽×1 格高**（`Workbench`+`WorkbenchRight` 配对方块、加宽靠 `WorkbenchTile.e00=2`、存读档零改动）、背包合成只留工作台配方、木剑/木箭/木镐入独立 `WorkbenchUI`、右键工作台开 UI、离开范围自动关。**3 轮修复**：①贴图 PPU 100→16（尺寸）②sprite pivot (0,0)→**(0.25,0.5)**（修悬空+碰撞错位，实测嵌入 2×1 footprint 接地）③加 `WorkbenchUI` 右上角 ✕ 关闭按钮（代码订阅 `Close()`）。MCP 独立验证：0 编译错误、绑定正确、对齐截图确认，[交付记录](../codex-reports/026b-workbench-rework.md)。
